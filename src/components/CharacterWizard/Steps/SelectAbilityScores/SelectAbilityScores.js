...
class SelectAbilityScores extends Component {
  ...
  render() {
    ...
    let abilityBonuses =
      this.props.abilityBonuses.length > 0
        ? this.props.abilityBonuses.slice()
        : [0, 0, 0, 0, 0, 0];
    const rows = [
      this.createRow(
        'Strength',
        `${str}`,
        `${abilityBonuses[0]}`,
        `${str ? Math.ceil((str -= 10) / 2) : 0}`,
        `${(str += abilityBonuses[0] + 10)}`,
      ),
      this.createRow(
        'Dexterity',
        `${dex}`,
        `${abilityBonuses[1]}`,
        `${dex ? Math.ceil((dex -= 10) / 2) : 0}`,
        `${(dex += abilityBonuses[1] + 10)}`,
      ),
      this.createRow(
        'Constitution',
        `${con}`,
        `${abilityBonuses[2]}`,
        `${con ? Math.ceil((con -= 10) / 2) : 0}`,
        `${(con += abilityBonuses[2] + 10)}`,
      ),
      this.createRow(
        'Intelligence',
        `${int}`,
        `${abilityBonuses[3]}`,
        `${int ? Math.ceil((int -= 10) / 2) : 0}`,
        `${(int += abilityBonuses[3] + 10)}`,
      ),
      this.createRow(
        'Wisdom',
        `${wis}`,
        `${abilityBonuses[4]}`,
        `${wis ? Math.ceil((wis -= 10) / 2) : 0}`,
        `${(wis += abilityBonuses[4] + 10)}`,
      ),
      this.createRow(
        'Charisma',
        `${cha}`,
        `${abilityBonuses[5]}`,
        `${cha ? Math.ceil((cha -= 10) / 2) : 0}`,
        `${(cha += abilityBonuses[5] + 10)}`,
      ),
    ];
    ...
    return (
      <div className="step-container">
        ...
        <p className="instructions">
          Roll four six-sided-dice (d6) and drop the lowest value. Add up the
          remaining three values and input the total for each ability. The table
          below will use your entries along with your racial bonuses to
          calculate your total ability score and modifier.
          <br />
